...
void CollisionSet::CollisionPushBack( const XMFLOAT3& size)
{
	for (int i = 0; i < JsonLoader::colliderObjects.size(); i++)
	{
		...
		colBox.LeastPos = { colBox.centerPos.x - colBox.size.x,colBox.centerPos.y - colBox.size.y,colBox.centerPos.z - colBox.size.z };

		XMFLOAT3 m1 = {0,0,0};
		
		//X
		if (colPlayer.MaxPos.x >= colBox.LeastPos.x)
		{
		...
		//Y
		//if (colPlayer.MaxPos.y >= colBox.LeastPos.y)
		//{
		...
		//Z
		if (colPlayer.MaxPos.z >= colBox.LeastPos.z)
		{
