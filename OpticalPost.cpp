...
std::vector<std::unique_ptr<Object3d>> OpticalPost::OpticalPostEffects;
...
void OpticalPost::Initialize()
{
	...
	for (int i = 0; i < OpticalPosts.size(); i++)
	{
		for (int j = 0; j < effectNum; j++)
		{
			...
			objectOpticalPost = Object3d::Create();
			objectOpticalPost->SetModel(modelOpticalPost);
			pos = OpticalPosts[i]->GetPosition();

			pos.y -= 80;
			pos.x += (rand() % (10 - (-10 + 1)) + (-10) );
			pos.z += (rand() % (5 - (-5 + 1)) + (-5));
			objectOpticalPost->SetPosition(pos);
			objectOpticalPost->SetScale({ 1.0f, 20.0f, 1.0f });
			OpticalPostEffects.push_back(std::move(objectOpticalPost));
		}
	}
...
void OpticalPost::Update(const XMFLOAT3& cameraPos)
{
	...
	for (int i = 0; i < OpticalPostEffects.size(); i++)
	{
		XMFLOAT3 crystal_CameraPos = { cameraPos.x, 0, cameraPos.z };
		XMFLOAT3 crystal_normal = { normal.x, 0, normal.z };
		////各取得アイテムの位置をベクトルに
		crystal_CameraPos.x -= OpticalPostEffects[i].get()->GetPosition().x;
		crystal_CameraPos.z -= OpticalPostEffects[i].get()->GetPosition().z;
		...
		if (OpticalPostEffects[i].get()->GetPosition().z > cameraPos.z)
		{
			sita = 360.0f - sita;
		}
		...
		OpticalPostEffects[i].get()->SetRotation({ 0, sita,0 });
	}
	...
	for (int i = 0; i < OpticalPostEffects.size(); i++)
	{
		OpticalPostEffects[i].get()->Update();
	}
}
...
void OpticalPost::Erase(int num)
{
	...
	{
		int Num = (num - 1) * effectNum;
		Num += i;
		OpticalPostEffects.erase(OpticalPosts.begin() + Num);
	}
}
