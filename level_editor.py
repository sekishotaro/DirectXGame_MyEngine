...
def draw_menu_manual(self, context):
    ...
    #トップばーの「エディターメニュー」に項目(オペレータ)を追加
    self.layout.operator("wm.url_open_preset", text="MyMenu", icon='HELP')
...
class TOPBAR_MT_my_menu(bpy.types.Menu):
    ...
    bl_label = "MyMenu"
    #著作表示用の文字列
    bl_bescription = "拡張メニュー by" + bl_info["author"]
...
class MYADDON_OT_export_scene(bpy.types.Operator, bpy_extras.io_utils.ExportHelper):
    ...
    def export_json(self):
        ...
        for object in bpy.context.scene.objects:
            ...
            self.parse_scene_recursice_json(json_object_root["objects"], object, 0)
    ...
    def parse_scene_recursive(self, file, object, level):
        ...
        if "type_name" in object:
            self.write_and_print(file, indent + "N %s" % object["type_name"])
    ...
    def parse_scene_recursice_json(self, data_parent, object, level):
        ...
        #カスタムプロパティ 'type_name'
        if "type_name" in object:
            json_object["type_name"] = object["type_name"]
        ...
        if len(object.children) > 0:
            ...
            #子ノードへ進む(深さが1上がる)
            for child in object.children:
                self.parse_scene_recursice_json(json_object["children"], child, level + 1)
...
class MYADDON_OT_add_filename(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        #パネル　ファイル名
class OBJECT_PT_type_name(bpy.types.Panel):
    """オブジェクトのタイプネームパネル"""
    bl_idname = "OBJECT_PT_type_name"
    bl_label = "TypeName"
    bl_space_type = "PROPERTIES"
    bl_region_type = "WINDOW"
    bl_context = "object"
    ...
    def draw(self, context):
        #パネルに項目を追加
        if "type_name" in context.object:
            #既にプロパティがあれば、プロパティを表示
            self.layout.prop(context.object, '["type_name"]', text=self.bl_label)
        else:
            #プロパティが無ければ、プロパティ追加ボタンを表示
            self.layout.operator(MYADDON_OT_add_typename.bl_idname)

#オペレータ　カスタムプロパティ['type_name']追加
class MYADDON_OT_add_typename(bpy.types.Operator):
    bl_idname = "myaddon.myaddon_ot_add_typename"
    bl_label = "TypeName 追加"
    bl_description = "['type_name'カスタムプロパティを追加します]"
    bl_options = {"REGISTER", "UNDO"}

    def execute(self, context):
        #['type_name']カスタムプロパティを追加
        context.object["type_name"] = ""
